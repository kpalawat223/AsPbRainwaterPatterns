...
# Dissolved Inorganic Analytes, Rainwater ----
...
#add unique identifier to each sample to avoid errors in widening sample data
iw$ID <- rownames(iw)

#remove already corrected data
iw <- iw[,-c(4,6)]
...
iw.raw.long <- full_join(iw.long, mlod.long[,c("mlodName", "analyte", "mlodValue")], by = c("mlodName", "analyte"))
...
iw.corrected.long$sampleValueCorrected <- ifelse(iw.corrected.long$value<=as.numeric(iw.corrected.long$mlodValue),
                                                        formatC(signif(((as.numeric(iw.corrected.long$mlodValue)/sqrt(2))),digits=2), digits=2,format="fg", flag="#"),
                                                        formatC(signif((iw.corrected.long$value),digits=4), digits=4,format="fg", flag="#"))

#remove unnecessary columns from data frame 
iw.corrected.long <- subset(iw.corrected.long, select = -c(ID, value, mlodValue))

iw.corrected.long[iw.corrected.long$analyte == "arsenic",]$analyte <- "arsenicCorrected"
iw.corrected.long[iw.corrected.long$analyte == "lead",]$analyte <- "leadCorrected"

#make data wide-form for printing to csv
iw.corrected.wide <- pivot_wider(data=iw.corrected.long,
                                 names_from = "analyte",
                                 values_from = "sampleValueCorrected"
